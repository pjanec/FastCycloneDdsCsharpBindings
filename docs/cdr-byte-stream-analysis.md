...
### 104. AtomicTests::SequenceBooleanTopic [PASS]
**IDL:** `@final @topic struct SequenceBooleanTopic { @key long id; sequence<boolean> values; };`
**Golden CDR Bytes:** `00 01 00 03 1c 02 00 00 01 00 00 00 01 00 00 00`
**Decoding:**
*   **Header:** `00 01 00 03` (XCDR1 LE).
*   **ID:** `1c 02...` (540). 4 bytes.
*   **Length:** `01 00 00 00` (1). 4 bytes.
*   **Value:** `01` (True). 1 byte.
*   **Padding:** `00 00 00` (3 bytes).
    *   *Reason:* Total message size must align. 4+4+4+1 = 13. Pad to 16 (4-byte alignment for XCDR1 struct).
*   **Total:** 16 bytes.

### 105. AtomicTests::SequenceOctetTopic [PASS]
**IDL:** `@final @topic struct SequenceOctetTopic { @key long id; sequence<octet> bytes; };`
**Golden CDR Bytes:** `00 01 00 03 26 02 00 00 01 00 00 00 28 00 00 00`
**Decoding:**
*   **Header:** `00 01 00 03`.
*   **ID:** `26 02...` (550). 4 bytes.
*   **Length:** `01 00...` (1). 4 bytes.
*   **Value:** `28` (40). 1 byte.
*   **Padding:** `00 00 00`. 3 bytes.
*   **Total:** 16 bytes.
...
### 107. AtomicTests::UnionBoolDiscTopic [PASS]
**IDL:** `@final union BoolUnion switch(boolean) { case TRUE: long true_val; ... };`
**Golden CDR Bytes:** `00 01 00 00 62 02 00 00 01 00 00 00 24 77 00 00`
**Decoding:**
*   **Header:** `00 01 00 00`.
*   **ID:** `62 02...` (610). 4 bytes.
*   **Discriminator:** `01 00 00 00` (TRUE).
    *   *Analysis:* Boolean is 1 byte. Next field is `long` (4 bytes).
    *   XCDR1 requires the `long` to be aligned to 4 bytes.
    *   Stream offset: 4(H) + 4(ID) = 8.
    *   Disc(1) + Pad(3) = 4 bytes. Offset becomes 12 (Aligned).
*   **Value:** `24 77 00 00` (30500). 4 bytes.
*   **Total:** 16 bytes.

### 108. AtomicTests::UnionLongDiscTopic [PASS]
**IDL:** `@final union SimpleUnion switch(long) { case 1: long int_val; ... };`
**Golden CDR Bytes:** `00 01 00 00 58 02 00 00 01 00 00 00 60 ea 00 00`
**Decoding:**
*   **ID:** `58 02...` (600). 4 bytes.
*   **Discriminator:** `01 00 00 00` (1). 4 bytes.
*   **Value:** `60 ea...` (60000). 4 bytes.
...
---
### 110. AtomicTests::UnionEnumDiscTopic [PASS]
**IDL:** `@final union ColorUnion switch(ColorEnum) { case RED: long red_data; ... };`
**Golden CDR Bytes:** `00 01 00 00 6c 02 00 00 00 00 00 00 70 30 00 00`
**Decoding:**
*   **ID:** `6c 02...` (620). 4 bytes.
*   **Discriminator:** `00 00 00 00` (RED). 4 bytes (Enums are 32-bit).
*   **Value:** `70 30 00 00` (12400). 4 bytes.
*   **Total:** 16 bytes.
