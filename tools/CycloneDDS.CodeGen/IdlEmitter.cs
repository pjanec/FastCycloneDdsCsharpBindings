...
namespace CycloneDDS.CodeGen
{
    public class IdlEmitter
    {
        public string EmitIdl(TypeInfo type)
        {
            ...
            // Includes
            var dependencies = GetTopicDependencies(type);
            ...
            // Emit dependent structs first (Reversed topological sort)
            var dependentStructs = GetDependentStructs(type);
            foreach(var structType in dependentStructs)
            {
        ...
        private HashSet<string> GetTopicDependencies(TypeInfo type)
        {
            var deps = new HashSet<string>();
            foreach (var field in type.Fields)
            {
                if (field.Type != null && field.Type.IsTopic)
                {
                ...
                if (field.GenericType != null && field.GenericType.IsTopic)
                {
                    if (field.GenericType.Name != type.Name)
                         deps.Add(field.GenericType.Name);
                }
            }
            return deps;
        }

        private List<TypeInfo> GetDependentStructs(TypeInfo root)
        {
            var visited = new HashSet<TypeInfo>();
            var sorted = new List<TypeInfo>();
        ...
        private void Visit(TypeInfo type, HashSet<TypeInfo> visited, List<TypeInfo> sorted)
        {
             foreach(var field in type.Fields)
             {
                 var target = field.Type;
                 if (target == null && field.GenericType != null)
                     target = field.GenericType;
